...
test("codeSafeVariableNameForToken_case_1", () => {
  ...
  expect(NamingHelper.codeSafeVariableNameForToken(token, StringCase.snakeCase, tokenGroup, "colors")).toBe("colors_brand_primary_red_1000")
})
...
test("codeSafeVariableNameForToken_case_2", () => {
  ...
  expect(NamingHelper.codeSafeVariableNameForToken(token, StringCase.pathCase, tokenGroup, null)).toBe("brand/red/red1000")
})
